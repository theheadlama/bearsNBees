...
class Boid {
  constructor(x, y) {
    ...
    this.size = 3.0;
    this.minimumLifeSpan = 5000; // in milliseconds
    ...
    this.maxForce = 0.05;
    this._maxForce = this.maxForce;
    colorMode(HSB);
    this.color = color(random(256), 255, 255);
  }

  run(boids) {
    if(this.isNearBeeHive()) {
        return false;   
    }
    this.flock(boids);
    this.update();
  ...
  flock(boids) {
    ...
    if (distanceToTarget < targetSeparation) {
        ...
        fleeForce.limit(this._maxForce);
  ...
  // Method to test for proximity to beehive
  isNearBeeHive() {
    let distanceToBeeHive = p5.Vector.dist(this.position, beeHivePosition);
    if(distanceToBeeHive < 35 && (millis() - this.birthTime) > this.minimumLifeSpan) {
      return true;
    }
    return false;
  }
  ...
  render() {
    ...
    rotate(theta);
    //image(graySquirrelImage, -20, 20);       
    //rotate(theta);
    // if(this.velocity.x < 0) {
    //     scale(-1,1);
    // }
    // image(brownSquirrelImage, -20, 20);
    imageMode(CENTER)
    image(beeImage, 0, 0);
